...
var g_time_out = 5000;//ajax请求超时时间为5秒
...
Util.jumpToApp = function(appId,areaCode,cityName) {
   ...
   if (typeof AlipayJSBridge != "undefined") {
    ...
    $.ajax({
        url: BASE_URL + '/sv/p/auth/appDispatcher',
        type: 'post',
        dataType: 'json',
...
Util.clickOnCard = function(cardId,url,areaCode,cityName) {
    ...
    $.ajax({
        url: BASE_URL + '/sv/p/auth/cardDispatcher',
        type: 'post',
        dataType: 'json',
        data: JSON.stringify(params),
        contentType: 'application/json; charset=UTF-8',
...
//分享后的回调
Util.shareCallback = function(result){
    if(!result) return;
    var bizId = result.shareResult ? 1 : 0;
    Util.log({
        "seedType":"sv",
        "seedId":"share_result",
        "bizId":bizId
...
// 汉字转拼音
Util.ConvertPinyin = function(l1) {
    var l2 = l1.length;
    var I1 = "";
...
//格式化显示时间
Util.formatTime = function(pTime) {
    if (!pTime) return "";
    ...
    if (pTime.isSame(today, "day")) {
        ...
        if (pTime.isSame(today, "year")) {
            ...
            //与今天不是同一年
            timeStr = pTime.format("YYYY-MM-DD HH:mm");
        }
    }
    return timeStr;
}
